...
def handle_ping_test(data):
    """
    Handle ping tests from the SocketTester component.
    Returns the timestamp received and additional diagnostic info.
    """
    session_id = request.sid
    session_info = player_sessions.get(session_id, {'is_authenticated': False, 'username': 'Unknown'})
    
    response = {
        'server_received_timestamp': time.time() * 1000,  # Convert to ms for JS
        'client_timestamp': data.get('timestamp'),
        'server_id': session_id,
        'connection_info': {
            'authenticated': session_info.get('is_authenticated', False),
            'username': session_info.get('username', 'Unknown'),
            'transport': request.namespace.transport_name
        }
    }
    
    print(f"Socket.IO: Ping test received from {session_id}")
    return response  # This is sent as acknowledgement
...
if __name__ == '__main__':
    ...
    # Import and register singleplayer handlers
